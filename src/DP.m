function DP( k, pbm, cst, data, agt )
% This function computes gridX containing the optimal forward states, as 
% well as gridV and gridP, respectively the optimal cost and error covariance
        ...
        for x3=1:data.x3Last,
            ...
            for u=1:data.uLast,
                ...
                if strcmp(pbm.sol,'group'),
                    s1Fwd = sFwd + agt.UAV1.D;
                    s2Fwd = sFwd + agt.UAV2.D;
                end
                ...
                if abs(x1Fwd)<data.X1(data.x1Last) && abs(x2Fwd)<data.X2(data.x2Last),
                    ...
                    if isempty(gridV{i1,i2,i3,k+1})==0,
                        ...
                        if strcmp(pbm.sol,'group'), sFwd = [s1Fwd s2Fwd]; end
                        P = getCost(k,pbm,agt,cst,oFwd,sFwd,xFwd);
